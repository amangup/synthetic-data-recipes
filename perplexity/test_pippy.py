...
llama = AutoModelForCausalLM.from_pretrained(
    "meta-llama/Llama-3.1-8B", low_cpu_mem_usage=True
)
print(llama)
...
mb_inputs = tokenizer(mb_prompts, return_tensors="pt", padding=True).to(device)['input_ids']

def cut_model_into_stage(model, stage_idx, num_stages):
    # pre decoder layers
    if stage_idx != 0:
    ...
    num_stages =  num_layers // num_stages
    start_idx = stage_idx * num_stages
    end_idx = num_layers if stage_idx == world_size-1 else (stage_idx+1) * num_stages

    model.model.layers = model.model.layers[start_idx:end_idx]
    
    if stage_idx == 0:
        return mb_inputs
    else:
        return torch.randn((mb_inputs.size(0), mb_inputs.size(1), model.config.hidden_size))
...
# pipe = pipeline(module=llama, mb_args=(mb_inputs["input_ids"],), split_spec=split_spec)

# Create pipeline stage for each rank
#stage = pipe.build_stage(rank, device=device)

example_input = cut_model_into_stage(llama, rank, world_size)
